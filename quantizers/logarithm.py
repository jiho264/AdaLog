...
class Log2Quantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_quant = (
            round_ste(-scaled_x.log2())
            if self.training_mode
            else torch.round(-scaled_x.log2())
        )
...
class AdaLogQuantizer(Log2Quantizer):
    ...
    def update_table(self):
        for i in range(0, self.n_levels * 2):
            val = round(
                (2 ** (-((self.q.item() * i) % self.r) / self.r))
                * (4 * self.n_levels - 2)
            ) / (4 * self.n_levels - 2)
            self.table1[i].data.copy_(
                torch.tensor(math.floor(i * self.q.item() / self.r))
            )
    ...
    def forward(self, x):
        ...
        if self.training_mode:
            ...
            x_quant = torch.clamp(x_quant, 0, 2 * self.n_levels - 1)
            x_dequant = (
                (2 ** (-self.table1[x_quant.long()]))
                * self.table2[x_quant.long()]
                * self.scale
            )
        x_dequant = x_dequant * mask
        return x_dequant
