...
class UniformQuantizer(nn.Module):
    def __init__(
        self, n_bits: int = 8, symmetric: bool = False, channel_wise: bool = False
    ):
        super().__init__()
        self.sym = symmetric
        self.n_bits = n_bits
    ...
    def forward(self, x):
        ...
        assert self.inited
        x_int = (
            round_ste(x / self.scale)
            if self.training_mode
            else torch.round(x / self.scale)
        )
        if self.sym:
            x_quant = x_int.clamp(-self.n_levels, self.n_levels - 1)
            x_dequant = x_quant * self.scale
        else:
            x_quant = (x_int + round_ste(self.zero_point)).clamp(
                0, 2 * self.n_levels - 1
            )
...
class TwinUniformQuantizer(UniformQuantizer):
    ...
    def forward(self, x):
        ...
        if self.training_mode:
            x_pos = (
                round_ste(x / (self.scale[0]))
                .clamp(0, self.n_levels - 1)
                .mul(self.scale[0])
            )
            x_neg = (
                round_ste(x / (self.scale[1]))
                .clamp(-self.n_levels, 0)
                .mul(self.scale[1])
            )
        else:
            x_pos = (
                torch.round(x / (self.scale[0]))
                .clamp(0, self.n_levels - 1)
                .mul(self.scale[0])
            )
            x_neg = (
                torch.round(x / (self.scale[1]))
                .clamp(-self.n_levels, 0)
                .mul(self.scale[1])
            )
