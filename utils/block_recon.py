...
class BlockReconstructor(QuantCalibrator):
    def __init__(self, model, full_model, calib_loader):
        ...
        for name, module in self.model.named_modules():
            if (
                any(isinstance(module, t) for t in types_of_block)
                or name.split(".")[-1] == "head"
            ):
        ...
        for name, module in block.named_modules():
            if hasattr(module, "w_quantizer"):
                if isinstance(module, MinMaxQuantLinear):
                    module.w_quantizer = AdaRoundQuantizer(
                        uq=module.w_quantizer,
                        weight_tensor=module.weight.view(
                            module.n_V, module.crb_rows, module.in_features
                        ),
    ...
    def init_block_raw_data(self, block, full_block, name, device, keep_gpu=True):
        ...
        if keep_gpu:
            block.raw_input, block.raw_out = block.raw_input.to(
                device
            ), block.raw_out.to(device)
    ...
    def reconstruct_single_block(
        ...
        for _name, module in block.named_modules():
            if hasattr(module, "mode"):
                if isinstance(module, MinMaxQuantLinear) or isinstance(
                    module, MinMaxQuantConv2d
                ):
        ...
        a_scheduler = (
            torch.optim.lr_scheduler.CosineAnnealingLR(
                a_optimizer, T_max=iters, eta_min=0.0
            )
            if len(a_params) != 0
            else None
        )
        ...
        for name, module in self.model.named_modules():
            ...
            if hasattr(module, "w_quantizer"):
                module.weight.data.copy_(
                    module.w_quantizer.get_hard_value(module.weight.data)
                )
