...
class MinMaxQuantConv2d(nn.Conv2d):
    ...
    def debug_only_quant_weight(self, x):
        w_sim, bias_sim = self.quant_weight_bias()
        out = F.conv2d(
            x, w_sim, bias_sim, self.stride, self.padding, self.dilation, self.groups
        )
        return out
...
class PTQSLQuantConv2d(MinMaxQuantConv2d):
    ...
    def quant_weight_bias(self):
        ...
        w_sim = self.w_quantizer(self.weight.view(oc, ic * kw * kh)).view(
            oc, ic, kw, kh
        )
        return w_sim, self.bias if self.bias is not None else None
...
class PTQSLBatchingQuantConv2d(PTQSLQuantConv2d):
    ...
    def _initialize_calib_parameters(self):
        ...
        numel = (
            2 * self.raw_input[: self.calib_batch_size].numel()
            + 2 * self.raw_out[: self.calib_batch_size].numel()
    ...
    def _initialize_activation_scale(self):
        ...
        tmp_a_scale = (
            torch.cat(tmp_a_scales, dim=1).amax(dim=1, keepdim=False).view(1, 1, 1, 1)
        )
    ...
    def _search_best_a_scale(self, input_scale_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                ) * cur_a_scale  # shape: parallel_eq_n,B,ic,iw,ih
...
class AsymmetricallyBatchingQuantConv2d(PTQSLBatchingQuantConv2d):
    ...
    def _search_best_w_scale(
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st + self.parallel_eq_n)
                cur_w_scale = weight_scale_candidates[
                    p_st:p_ed
                ]  # shape: (parallel_eq_n, 1, 1) or (parallel_eq_n, oc, 1)
                cur_w_zero_point = weight_zero_point_candidates[p_st:p_ed]
                # quantize weight and bias
                oc, ic, kw, kh = self.weight.data.shape
                w_sim = self.weight.view(oc, -1).unsqueeze(
                    0
                )  # shape: (1, oc, ic*kw*kh)
                w_quant = ((w_sim / cur_w_scale).round_() + cur_w_zero_point).clamp(
                    0, 2 * self.w_quantizer.n_levels - 1
                )
                w_sim = (w_quant - cur_w_zero_point).mul_(
                    cur_w_scale
                ...
                similarity = torch.sum(
                    similarity, dim=0, keepdim=True
                )  # shape: (1,parallel_eq_n) or (1,parallel_eq_n,oc)
                similarities.append(similarity)
            similarities = torch.cat(
                similarities, dim=1
            )  # shape: (1,eq_n) or (1,eq_n,oc)
            batch_similarities.append(similarities)
        batch_similarities = torch.cat(batch_similarities, dim=0).sum(
            dim=0, keepdim=False
        )  # shape: (eq_n) or (eq_n,oc)
    ...
    def calculate_percentile_weight_candidates(self, l=0.9, r=1.0):
        ...
        w_lowers_candidates = torch.quantile(
            ...
            -1
        )  # shapeL 2, out_channels, 1
        delta_min = w_uppers_candidates[0:1] - w_lowers_candidates[0:1]
        delta_max = w_uppers_candidates[1:] - w_lowers_candidates[1:]
        splits = torch.linspace(0, 1, steps=num_scale).cuda()[:, None, None] * (
            delta_max - delta_min
        )
        weight_scale_candidates = (delta_min + splits).repeat(num_zp, 1, 1) / (
    ...
    def weight_fpcs(self, fpcs_width=16, steps=4, search_strategy=None):
        ...
        while remain_steps > 0:
            delta_scale_candidates = (
                torch.linspace(0, 1, steps=fpcs_new_cnt).cuda()[:, None, None] - 0.5
            ) * delta_scale
    ...
    def hyperparameter_searching(self):
        ...
        for e in range(self.search_round):
            if self.fpcs:
                self.weight_fpcs(
                    steps=self.steps,
                    search_strategy=AsymmetricallyBatchingQuantConv2d._search_best_w_scale,
                )
            else:
