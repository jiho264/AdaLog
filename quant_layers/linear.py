...
class MinMaxQuantLinear(nn.Linear):
    def __init__(
        ...
        self.mode = mode
        self.w_quantizer = UniformQuantizer(
            n_bits=w_bit, symmetric=True, channel_wise=False
        )
        self.a_quantizer = UniformQuantizer(
            n_bits=a_bit, symmetric=True, channel_wise=False
        )
        self.raw_input = None
        self.raw_out = None
        self.tmp_input = None
    ...
    def quant_forward(self, x):
        assert (
            self.calibrated
        ), f"Module should be calibrated before run quant_forward for {self}"
...
class PTQSLQuantLinear(MinMaxQuantLinear):
    def __init__(
        ...
        super().__init__(
            in_features, out_features, bias=bias, mode=mode, w_bit=w_bit, a_bit=a_bit
        )
        self.w_quantizer = UniformQuantizer(
            n_bits=w_bit, symmetric=True, channel_wise=True
        )
        self.a_quantizer = UniformQuantizer(
            n_bits=a_bit, symmetric=True, channel_wise=False
        )
    ...
    def quant_weight_bias(self):
        w_sim = self.w_quantizer(
            self.weight.view(self.n_V, self.crb_rows, self.in_features)
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    ...
    def _initialize_calib_parameters(self):
        ...
        numel = (
            8 * self.raw_input[: self.calib_batch_size].numel()
            + 16 * self.raw_out[: self.calib_batch_size].numel()
        )  # number of parameters on GPU
        self.parallel_eq_n = int((memory / 4) // numel)
        self.parallel_eq_n = math.ceil(
            self.eq_n * 1.0 / math.ceil(self.eq_n * 1.0 / self.parallel_eq_n)
        )
    ...
    def _initialize_activation_scale(self):
        ...
        for b_st in range(0, self.raw_input.shape[0], self.calib_batch_size):
            ...
            a_scale_ = (
                (x_.abs().max() / (self.a_quantizer.n_levels - 0.5)).detach().view(1, 1)
            )
    ...
    def _search_best_w_scale(self, weight_scale_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            )  # shape: b,*,1,out_features
            raw_out_expanded = raw_out_expanded.view(
                *raw_out_expanded.shape[:-1], self.n_V, -1
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                w_sim = self.weight.view(
                    self.n_V, self.crb_rows, self.in_features
                ).unsqueeze(
                    0
                )  # shape: 1,n_V,crb_rows,in_features
                w_sim = (
                    (w_sim / cur_w_scale)
                    .round_()
                    .clamp_(-self.w_quantizer.n_levels, self.w_quantizer.n_levels - 1)
                    .mul_(cur_w_scale)
                )  # shape: parallel_eq_n,n_V,crb_rows,in_features
                w_sim = w_sim.view(
                    -1, self.in_features
                )  # shape: parallel_eq_n*out_features,in_features
                bias_sim = (
                ...
                )  # shape: b,*,parallel_eq_n*out_features
                out_sim = out_sim.view(
                    *out_sim.shape[:-1], p_ed - p_st, self.n_V, -1
                )  # shape: b,*,parallel_eq_n,n_V,crb_rows
                similarity = self._get_similarity(
                    raw_out_expanded, out_sim
                )  # shape: b,*,parallel_eq_n,n_V,crb_rows
                if len(similarity.shape) > 4:
                    similarity = torch.mean(
                        similarity, dim=list(range(1, len(similarity.shape) - 3))
                    )  # shape: b,parallel_eq_n,n_V,crb_rows
                similarity = similarity.sum(
                    dim=0, keepdim=True
                )  # shape: (1, parallel_eq_n, n_V) or (1, parallel_eq_n, n_V, crb_rows)
                similarities.append(similarity)
            similarities = torch.cat(
                similarities, dim=1
            )  # shape: (1, eq_n, n_V) or (1, eq_n, n_V, crb_rows)
            batch_similarities.append(similarities)
        batch_similarities = torch.cat(batch_similarities, dim=0).sum(
            dim=0, keepdim=False
        )  # shape: (eq_n, n_V) or (eq_n, n_V, crb_rows)
        best_index = batch_similarities.argmax(dim=0).reshape(
            1, self.n_V, -1, 1
        )  # shape: (1,n_V,1,1) or (1,n_V,crb_rows,1)
        tmp_w_scale = torch.gather(
            weight_scale_candidates, dim=0, index=reshaped_best_index
        )  # shape: (1,n_V*crb_rows,1)
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_sim = (
                    (x_sim / cur_a_scale)
                    .round_()
                    .clamp_(-self.a_quantizer.n_levels, self.a_quantizer.n_levels - 1)
                    .mul_(cur_a_scale)
                )  # shape: B,*,in_features,parallel_eq_n
                x_sim = x_sim.permute(
                    *list(range(len(x_sim.shape) - 2)), -1, -2
                )  # shape: B,*,parallel_eq_n,in_features
    ...
    def hyperparameter_searching(self):
        ...
        ).cuda().view(-1, 1, 1, 1) * self.w_quantizer.scale.unsqueeze(
            0
        )  # shape: eq_n,n_V,1,1
        ...
        ).cuda().view(1, -1) * self.a_quantizer.scale.unsqueeze(
            -1
        )  # shape: (1,eq_n) or (in_features,eq_n)
...
class AsymmetricallyBatchingQuantLinear(PTQSLBatchingQuantLinear):
    def __init__(
        ...
        self.w_quantizer = UniformQuantizer(
            n_bits=w_bit, symmetric=False, channel_wise=True
        )
        self.a_quantizer = UniformQuantizer(
            n_bits=a_bit, symmetric=False, channel_wise=False
        )
        self.a_quantizer.scale = nn.Parameter(torch.zeros((1)))
    ...
    def _initialize_weight_scale(self):
        self.w_quantizer.scale.data.copy_(
            (
                self.weight.view(self.n_V, self.crb_rows, self.in_features).amax(
                    [2], keepdim=True
                )
                - self.weight.view(self.n_V, self.crb_rows, self.in_features).amin(
                    [2], keepdim=True
                )
            )
    ...
    def _initialize_activation_scale(self):
        ...
        for b_st in range(0, self.raw_input.shape[0], self.calib_batch_size):
            ...
            if self.a_quantizer.channel_wise:
                a_max = (
                    x_.abs()
                    .amax([i for i in range(x_.ndim - 1)], keepdim=False)
                    .detach()
                    .view(1, -1)
                )
                a_min = (
                    x_.abs()
                    .amin([i for i in range(x_.ndim - 1)], keepdim=False)
                    .detach()
                    .view(1, -1)
                )
            else:
        ...
        self.a_quantizer.scale.data.copy_(
            (tmp_a_max - tmp_a_min) / (2 * self.a_quantizer.n_levels - 1)
        )
        self.a_quantizer.zero_point.data.copy_(-tmp_a_min / self.a_quantizer.scale)
        self.a_quantizer.inited = True

    def _search_best_w_scale_self(
        self, weight_scale_candidates, weight_zero_point_candidates, topk=1
    ):
        similarities = []
        raw_weight = self.weight.view(
            self.n_V, self.crb_rows, self.in_features
        ).unsqueeze(
            0
        )  # shape: 1,n_V,crb_rows,in_features
        for p_st in range(0, self.eq_n, self.parallel_eq_n):
            ...
            # quantize weight and bias
            w_quant = ((raw_weight / cur_w_scale).round_() + cur_w_zero_point).clamp(
                0, 2 * self.w_quantizer.n_levels - 1
            )
            w_dequant = (
                w_quant - cur_w_zero_point
            ) * cur_w_scale  # shape: parallel_eq_n,n_V,crb_rows,in_features
            similarity = self._get_similarity(
                raw_weight, w_dequant
            )  # shape: parallel_eq_n,n_V,crb_rows,in_features
            similarity = torch.mean(
                similarity, dim=-1, keepdim=False
            )  # shape: parallel_eq_n,n_V,crb_rows
            similarities.append(similarity)
        ...
        if topk == 1:
            ...
            tmp_w_zero_point = torch.gather(
                weight_zero_point_candidates, dim=0, index=best_index
            )
            self.w_quantizer.scale.data.copy_(tmp_w_scale.squeeze(0))
    ...
    def _search_best_a_scale_self(
        self, input_scale_candidates, input_zero_point_candidates, topk=1
    ):
        batch_similarities = (
            []
        )  # similarities, need to concatenate and calculate sum (equivalent to mean with argmax)
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            x = self.raw_input[b_st:b_ed].cuda()
            raw_x = (
                self.raw_input[b_st:b_ed].cuda().unsqueeze(-1)
            )  # shape: b,*,in_features,1
            similarities = []
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st + self.parallel_eq_n)
                cur_a_scale = input_scale_candidates[:, p_st:p_ed]
                ...
                # quantize input
                x_sim = x.unsqueeze(-1)  # shape: B,*,in_features,1
                x_quant = ((x_sim / cur_a_scale).round_() + cur_a_zero_point).clamp_(
                    0, 2 * self.a_quantizer.n_levels - 1
                )  # shape: B,*,in_features,parallel_eq_n
                x_dequant = (
                    x_quant - cur_a_zero_point
                ) * cur_a_scale  # shape: B,*,in_features,parallel_eq_n
                similarity = self._get_similarity(
                    raw_x, x_dequant
                )  # shape: b,*,in_features,parallel_eq_n
                if len(similarity.shape) > 3:
                    similarity = torch.mean(
                        similarity, dim=list(range(1, len(similarity.shape) - 2))
                    )  # shape: b, in_features, parallel_eq_n
                if not self.a_quantizer.channel_wise:
                    similarity = torch.mean(
                        similarity, dim=1, keepdim=True
                    )  # shape: b, 1, parallel_eq_n
                similarity = torch.sum(
                    similarity, dim=0, keepdim=True
                )  # shape: 1, in_features, parallel_eq_n
                similarities.append(similarity)
            # store best input scale and store in tmp_a_scale
            similarities = torch.cat(
                similarities, dim=-1
            )  # shape: 1, in_features, eq_n
            batch_similarities.append(similarities)
        batch_similarities = torch.cat(batch_similarities, dim=0).sum(
            dim=0, keepdim=False
        )  # shape: in_features, eq_n
        _, best_index = torch.topk(
            batch_similarities, k=topk, dim=-1
        )  # shape: in_features, topk
        if topk == 1:
            tmp_a_scale = torch.gather(input_scale_candidates, dim=-1, index=best_index)
            tmp_a_zero_point = torch.gather(
                input_zero_point_candidates, dim=-1, index=best_index
            )
            self.a_quantizer.scale.data.copy_(tmp_a_scale.squeeze(-1))
    ...
    def _search_best_w_scale(
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                w_quant = ((w_sim / cur_w_scale).round_() + cur_w_zero_point).clamp(
                    0, 2 * self.w_quantizer.n_levels - 1
                )
                w_dequant = (
                    w_quant - cur_w_zero_point
                ) * cur_w_scale  # shape: parallel_eq_n,n_V,crb_rows,in_features
                w_sim = w_dequant.view(
                    -1, self.in_features
                )  # shape: parallel_eq_n*out_features,in_features
                bias_sim = (
                    self.bias.repeat(p_ed - p_st) if self.bias is not None else None
    ...
    def calculate_percentile_weight_candidates(self, l=0.9, r=1.0):
        ...
        w_uppers_candidates = torch.quantile(
            self.weight.view(self.n_V, self.crb_rows, self.in_features),
            pct.to(self.weight.device),
            dim=-1,
        ).unsqueeze(
            -1
        )  # shape: 2, n_V, crb_rows, 1
        w_lowers_candidates = torch.quantile(
            self.weight.view(self.n_V, self.crb_rows, self.in_features),
            (1 - pct).to(self.weight.device),
            dim=-1,
        ).unsqueeze(
            -1
        )  # shapeL 2, n_V, crb_rows, 1
        delta_min = w_uppers_candidates[0:1] - w_lowers_candidates[0:1]
        delta_max = w_uppers_candidates[1:] - w_lowers_candidates[1:]
        splits = torch.linspace(0, 1, steps=num_scale).cuda()[:, None, None, None] * (
            delta_max - delta_min
        )
        weight_scale_candidates = (delta_min + splits).repeat(num_zp, 1, 1, 1) / (
            2 * self.w_quantizer.n_levels - 1
        )
        ...
        weight_zero_point_candidates = zp_candidates.repeat_interleave(num_scale)[
            :, None, None, None
        ]
        weight_zero_point_candidates = weight_zero_point_candidates.repeat(
            1, self.n_V, self.crb_rows, 1
        )
    ...
    def calculate_percentile_activation_candidates(self, l=0.9, r=1.0):
        ...
        mini_batch_size = 1
        if self.a_quantizer.channel_wise:
            a_uppers_candidates = torch.quantile(
                x.view(-1, x.shape[-1]), pct.to(x.device), dim=0
            ).transpose(
                0, 1
            )  # shape: in_features, 2
            a_lowers_candidates = torch.quantile(
                x.view(-1, x.shape[-1]), (1 - pct).to(x.device), dim=0
            ).transpose(
                0, 1
            )  # shape: in_features, 2
        else:
            while tensor_too_large:
                try:
                    a_uppers_candidates = (
                        torch.quantile(
                            x.view(mini_batch_size, -1), pct.to(x.device), dim=-1
                        )
                        .mean(dim=-1)
                        .unsqueeze(0)
                    )  # shape: 1, 2
                    a_lowers_candidates = (
                        torch.quantile(
                            x.view(mini_batch_size, -1), (1 - pct).to(x.device), dim=-1
                        )
                        .mean(dim=-1)
                        .unsqueeze(0)
                    )  # shape: 1, 2
        ...
        splits = torch.linspace(0, 1, steps=num_scale).cuda()[None, :] * (
            delta_max - delta_min
        )
        a_scale_candidates = (
            (delta_min + splits).repeat(1, num_zp) / (2 * self.a_quantizer.n_levels - 1)
        ).clamp(min=1e-4)
        ...
        a_zero_point_candidates = a_zero_point_candidates.repeat(
            a_scale_candidates.shape[0], 1
        )
    ...
    def weight_fpcs(self, fpcs_width=16, steps=6, search_strategy=None):
        ...
        weight_scale_candidates, weight_zero_point_candidates = (
            self.calculate_percentile_weight_candidates()
        )
        delta_scale = weight_scale_candidates[1:2] - weight_scale_candidates[0:1]
        topk_index = search_strategy(
            self, weight_scale_candidates, weight_zero_point_candidates, topk=fpcs_width
        )
        topk_scale_candidates = torch.gather(
            weight_scale_candidates, dim=0, index=topk_index
        )
        topk_zp_candidates = torch.gather(
            weight_zero_point_candidates, dim=0, index=topk_index
        )
        remain_steps = steps - 1
        while remain_steps > 0:
            delta_scale_candidates = (
                torch.linspace(0, 1, steps=fpcs_new_cnt).cuda()[:, None, None, None]
                - 0.5
            ) * delta_scale
            delta_scale = delta_scale / (fpcs_new_cnt - 0.5)
            weight_scale_candidates = (
                topk_scale_candidates.unsqueeze(1) + delta_scale_candidates.unsqueeze(0)
            ).reshape(-1, *weight_scale_candidates.shape[1:])
            weight_zero_point_candidates = topk_zp_candidates.repeat_interleave(
                fpcs_new_cnt, dim=0
            )
            topk_index = search_strategy(
                self,
                weight_scale_candidates,
                weight_zero_point_candidates,
    ...
    def activation_fpcs(self, fpcs_width=16, steps=6, search_strategy=None):
        fpcs_new_cnt = int(self.eq_n / fpcs_width)
        a_scale_candidates, a_zero_point_candidates = (
            self.calculate_percentile_activation_candidates()
        )
        delta_scale = a_scale_candidates[:, 1:2] - a_scale_candidates[:, 0:1]
        topk_index = search_strategy(
            self, a_scale_candidates, a_zero_point_candidates, topk=fpcs_width
        )
        topk_scale_candidates = torch.gather(
            a_scale_candidates, dim=-1, index=topk_index
        )
        topk_zp_candidates = torch.gather(
            a_zero_point_candidates, dim=-1, index=topk_index
        )
        remain_steps = steps - 1
        while remain_steps > 0:
            delta_scale_candidates = (
                torch.linspace(0, 1, steps=fpcs_new_cnt).cuda()[None, :] - 0.5
            ) * delta_scale
            delta_scale = delta_scale / (fpcs_new_cnt - 0.5)
            a_scale_candidates = (
                (
                    topk_scale_candidates.unsqueeze(-1)
                    + delta_scale_candidates.unsqueeze(-2)
                )
                .reshape(*a_scale_candidates.shape[:-1], -1)
                .clamp(min=1e-4)
            )
            a_zero_point_candidates = topk_zp_candidates.repeat_interleave(
                fpcs_new_cnt, dim=-1
            )
            topk_index = search_strategy(
                self,
                a_scale_candidates,
                a_zero_point_candidates,
                topk=1 if remain_steps == 1 else fpcs_width,
            )
            if remain_steps > 1:
                topk_scale_candidates = torch.gather(
    ...
    def hyperparameter_searching(self):
        ...
        if self.fpcs:
            self.weight_fpcs(
                steps=self.steps,
                search_strategy=AsymmetricallyBatchingQuantLinear._search_best_w_scale_self,
            )
            self.activation_fpcs(
                steps=self.steps,
                search_strategy=AsymmetricallyBatchingQuantLinear._search_best_a_scale_self,
            )
        else:
            weight_scale_candidates, weight_zero_point_candidates = (
                self.calculate_percentile_weight_candidates()
            ...
            self._search_best_w_scale_self(
                weight_scale_candidates, weight_zero_point_candidates
            )
        ...
        for e in range(self.search_round):
            if self.fpcs:
                self.weight_fpcs(
                    steps=self.steps,
                    search_strategy=AsymmetricallyBatchingQuantLinear._search_best_w_scale,
                )
                self.activation_fpcs(
                    steps=self.steps,
                    search_strategy=AsymmetricallyBatchingQuantLinear._search_best_a_scale,
                )
            else:
                self._search_best_w_scale(
                    weight_scale_candidates, weight_zero_point_candidates
                )
...
class AsymmetricallyChannelWiseBatchingQuantLinear(AsymmetricallyBatchingQuantLinear):
    def __init__(
        ...
        self.a_quantizer = UniformQuantizer(
            n_bits=a_bit, symmetric=False, channel_wise=True
        )
    ...
    def reparam_step1(self):
        ...
        target_channel_zero_point = (
            torch.mean(self.a_quantizer.zero_point).round().view(1)
        )
        ...
        if self.bias is not None:
            self.bias.data = self.bias.data + torch.mm(
                self.weight.data, b.reshape(-1, 1)
            ).reshape(-1)
        else:
...
class PostGeluTwinUniformBatchingQuantLinear(AsymmetricallyBatchingQuantLinear):
    def __init__(
        ...
        self.a_quantizer = TwinUniformQuantizer(
            n_bits=a_bit, symmetric=False, channel_wise=False
        )
    ...
    def _initialize_activation_scale(self):
        ...
        for b_st in range(0, self.raw_input.shape[0], self.calib_batch_size):
            ...
            x_ = self.raw_input[b_st:b_ed].cuda()
            a_scale_ = (
                (x_.abs().max() / (self.a_quantizer.n_levels - 0.5))
                .detach()
                .view(1, 1)
                .expand(1, self.a_quantizer.scale.shape[-1])
            )
            tmp_a_scales.append(a_scale_)
        tmp_a_scale = torch.cat(tmp_a_scales, dim=0).amax(dim=0, keepdim=False).view(-1)
        a_neg = (
            torch.tensor(
                0.16997124254703522 / self.a_quantizer.n_levels,
                device=self.a_quantizer.scale.device,
            )
            .detach()
            .view(1)
            .repeat(self.a_quantizer.scale.shape[-1])
        )
    ...
    def _search_best_a_scale(self, input_scale_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                # quantize input
                x_sim = x.unsqueeze(-1)  # shape: B,*,in_features,1
                x_pos = (x_sim / cur_a_scale).round_().clamp_(
                    0, self.a_quantizer.n_levels - 1
                ) * (
                    cur_a_scale
                )  # shape: B,*,in_features,parallel_eq_n
                x_neg = (
                    x_sim / (self.a_quantizer.scale[1].unsqueeze(-1))
                ).round_().clamp_(
                    -self.a_quantizer.n_levels, 0
                ) * self.a_quantizer.scale[
                    1
                ].unsqueeze(
                    -1
                )  # shape: B,*,in_features,1
                x_sim = (x_pos + x_neg).permute(
                    *list(range(len(x_sim.shape) - 2)), -1, -2
                )  # shape: B,*,parallel_eq_n,in_features
                # calculate similarity and store them
                out_sim = F.linear(
    ...
    def hyperparameter_searching(self):
        ...
        ).cuda().view(1, -1) * self.a_quantizer.scale[1].unsqueeze(
            -1
        )  # shape: 1,eq_n
...
class PostGeluLogBasedBatchingQuantLinear(AsymmetricallyBatchingQuantLinear):
    ...
    def __init__(
        ...
        del self.a_quantizer
        self.a_quantizer = ShiftAdaLogQuantizer(
            n_bits=a_bit, symmetric=False, channel_wise=False
        )
        self.a_quantizer.scale = nn.Parameter(torch.zeros((1)))
        ...
        self.table_scale = 1.0 / (4 * self.a_quantizer.n_levels - 2)
        self.table = torch.round(self.table / self.table_scale) * self.table_scale

        if quantizer == "log2":
            self.tmp_quantizer = ShiftLog2Quantizer(
                n_bits=a_bit, symmetric=False, channel_wise=False
            )
            ...
            self.tmp_quantizer = ShiftLogSqrt2Quantizer(
                n_bits=a_bit, symmetric=False, channel_wise=False
            )
            self.tmp_quantizer.scale = nn.Parameter(torch.zeros((1)))
            self.tmp_quantizer.shift.data.copy_(torch.tensor(0.16997124254703522))
    ...
    def positive_percentile(tensor, q, dim=0):
        ...
        positive_tensor = torch.where(
            positive_mask, tensor, torch.tensor(float("nan")).to(tensor.device)
        )
        ...
        expanded_sorted_tensor = sorted_tensor.unsqueeze(0).expand(
            q.numel(), *sorted_tensor.shape
        )
    ...
    def calculate_percentile_activation_candidates(self, l=0.9, r=1.0):
        ...
        while True:
            ...
            try:
                candidates = (
                    PostGeluLogBasedBatchingQuantLinear.positive_percentile(
                        raw_inp, torch.tensor([l, r]).cuda()
                    )
                    + self.a_quantizer.shift.item()
                )  # shape: (2)
                break
            except:
                bc = bc // 2
        candidates = candidates.unsqueeze(0)
        input_scale_candidates = candidates[:, 0:1] + (
            candidates[:, 1:] - candidates[:, 0:1]
        ) * torch.tensor([i / (self.eq_n - 1) for i in range(self.eq_n)]).cuda().view(
            1, -1
        )
    ...
    def _search_best_a_scale(self, input_scale_candidates, topk=1):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                # quantize input
                x_sim = x.unsqueeze(-1)  # shape: B,*,in_features,1
                shifted_x_sim = ((x_sim + self.a_quantizer.shift) / cur_a_scale).clamp(
                    min=1e-15, max=1.0
                )
                x_sim_quant = torch.round(
                    -shifted_x_sim.log2() * self.a_quantizer.r / self.a_quantizer.q
                )
                mask = x_sim_quant >= 2 * self.a_quantizer.n_levels
                x_sim_quant = x_sim_quant.clamp_(0, 2 * self.a_quantizer.n_levels - 1)
                index = (
                    torch.remainder(
                        x_sim_quant * self.a_quantizer.q, self.a_quantizer.r
                    )
                    .round_()
                    .long()
                )
                ...
                ) * self.table.to(x.device)[index]
                x_sim[mask] = 0
                x_sim = (x_sim * cur_a_scale - self.a_quantizer.shift).permute(
                    *list(range(len(x_sim.shape) - 2)), -1, -2
                )
                out_sim = F.linear(
                    x_sim, w_sim, bias_sim
    ...
    def _search_best_log_base(self, q_candidates=None, topk=1):
        if q_candidates is None:
            q_candidates = (
                torch.tensor([i for i in range(10, 11 + self.eq_n)]).cuda().view(1, -1)
            )
        batch_similarities = (
            []
        )  # similarities, need to concatenate and calculate sum (equivalent to mean with argmax)
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_sim = x.unsqueeze(-1)  # shape: B,*,in_features,1
                shifted_x_sim = (
                    (x_sim + self.a_quantizer.shift) / self.a_quantizer.scale
                ).clamp(min=1e-15, max=1.0)
                x_sim_quant = torch.round(
                    -shifted_x_sim.log2() * self.a_quantizer.r / cur_q
                )
                mask = x_sim_quant >= 2 * self.a_quantizer.n_levels
                x_sim_quant = x_sim_quant.clamp_(0, 2 * self.a_quantizer.n_levels - 1)
                col_index = (
                    torch.remainder(x_sim_quant * cur_q, self.a_quantizer.r)
                    .round_()
                    .long()
                )
                x_sim = (
                    2 ** (-1 * torch.floor(x_sim_quant * cur_q / self.a_quantizer.r))
                ) * self.table.to(x.device)[col_index]
                x_sim[mask] = 0
                x_sim = (
                    x_sim * self.a_quantizer.scale - self.a_quantizer.shift
                ).permute(*list(range(len(x_sim.shape) - 2)), -1, -2)
                out_sim = F.linear(
                    x_sim, w_sim, bias_sim
                )  # shape: b,*,parallel_eq_n,out_features
                similarity = self._get_similarity(
    ...
    def activation_fpcs(
        self, ud_candidates, base_num=8, scale_num=16, fpcs_width=32, steps=6
    ):
        fpcs_new_cnt = int(self.eq_n / fpcs_width)
        q_candidates = (
            torch.tensor([i for i in range(10, 11 + self.eq_n)]).cuda().view(1, -1)
        ...
        a_scale_candidates = ud_candidates[:, 0:1] + (
            ud_candidates[:, 1:] - ud_candidates[:, 0:1]
        ) * torch.tensor([i / (scale_num - 1) for i in range(scale_num)]).cuda().view(
            1, -1
        )
        delta_scale = a_scale_candidates[:, 1:2] - a_scale_candidates[:, 0:1]
        ...
        topk_index = self._search_best_scale_logbase(
            a_scale_candidates, q_candidates, topk=fpcs_width
        )
        topk_scale_candidates = torch.gather(
            a_scale_candidates, dim=-1, index=topk_index
        )
    ...
    def hyperparameter_searching(self):
        ...
        ud_candidates, input_scale_candidates = (
            self.calculate_percentile_activation_candidates()
        )
