...
class MinMaxQuantMatMul(nn.Module):
    def __init__(self, A_bit=8, B_bit=8, mode="raw"):
        ...
        self.B_quantizer = UniformQuantizer(
            n_bits=B_bit, symmetric=True, channel_wise=False
        )
        self.raw_input = None
        self.raw_out = None
        self.tmp_input = None
...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def __init__(
        ...
        self.A_quantizer = UniformQuantizer(
            n_bits=A_bit, symmetric=True, channel_wise=head_channel_wise
        )
        self.B_quantizer = UniformQuantizer(
            n_bits=B_bit, symmetric=True, channel_wise=head_channel_wise
        )
        ...
        target_shape = (
            [1, self.num_heads, 1, 1] if self.head_channel_wise else [1, 1, 1, 1]
        )
...
class PTQSLBatchingQuantMatMul(PTQSLQuantMatMul):
    def __init__(
        ...
        super().__init__(
            A_bit, B_bit, mode, search_round, eq_n, head_channel_wise, num_heads
        )
    ...
    def _initialize_calib_parameters(self):
        ...
        numel = (
            4 * self.raw_input[0][: self.calib_size].numel()
            + 4 * self.raw_input[1][: self.calib_size].numel()
            + 8 * self.raw_out[: self.calib_batch_size].numel()
...
class AsymmetricallyBatchingQuantMatMul(PTQSLBatchingQuantMatMul):
    def __init__(
        ...
        self.A_quantizer = UniformQuantizer(
            n_bits=A_bit, symmetric=False, channel_wise=head_channel_wise
        )
        self.B_quantizer = UniformQuantizer(
            n_bits=B_bit, symmetric=False, channel_wise=head_channel_wise
        )
    ...
    def _search_best_A_scale(self, A_scale_candidates, A_zero_point_candidates, topk=1):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                A_sim = A.squeeze(0)
                A_quant = ((A_sim / cur_A_scale).round_() + cur_A_zero_point).clamp(
                    0, 2 * self.A_quantizer.n_levels - 1
                )
                A_sim = (A_quant - cur_A_zero_point).mul_(
                    cur_A_scale
                )  # shape: (parallel_eq_n,b,*,dim1,dim2)
                out_sim = A_sim @ B_sim  # shape: parallel_eq_n,b,*,dim1,dim3
                similarity = self._get_similarity(
                    raw_out, out_sim
                )  # shape: parallel_eq_n,b,*,dim1,dim3
                if self.head_channel_wise:
                    similarity = torch.mean(
                        similarity, dim=list(range(3, len(similarity.shape)))
                    )  # shape: parallel_eq_n,b,heads
                else:
                    similarity = torch.mean(
                        similarity, dim=list(range(2, len(similarity.shape)))
                    )  # shape: parallel_eq_n,b
                similarity = similarity.sum(
                    dim=1, keepdim=True
                )  # shape: (parallel_eq_n,1) or (parallel_eq_n,1,heads)
                similarities.append(similarity)
            # calculate best similarity for this block
            similarities = torch.cat(
                similarities, 0
            )  # shape: (eq_n,1) or (eq_n,1,heads)
            batch_similarities.append(similarities)
        batch_similarities = torch.cat(batch_similarities, dim=1).sum(
            dim=1, keepdim=False
        )  # shape: eq_n or (eq_n,heads)
        ...
        if topk == 1:
            ...
            tmp_A_zero_point = torch.gather(
                A_zero_point_candidates, dim=0, index=best_index
            )
            self.A_quantizer.scale.data.copy_(
                tmp_A_scale.view(self.A_quantizer.scale.shape)
            )
            self.A_quantizer.zero_point.copy_(
                tmp_A_zero_point.view(self.A_quantizer.zero_point.shape)
            )
        return best_index
    ...
    def _search_best_B_scale(self, B_scale_candidates, B_zero_point_candidates, topk=1):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                B_sim = B.squeeze(0)
                B_quant = ((B_sim / cur_B_scale).round_() + cur_B_zero_point).clamp(
                    0, 2 * self.B_quantizer.n_levels - 1
                )
                B_sim = (B_quant - cur_B_zero_point).mul_(
                    cur_B_scale
                )  # shape: (parallel_eq_n,b,*,dim2,dim3)
                out_sim = A_sim @ B_sim  # shape: parallel_eq_n,b,*,dim1,dim3
                similarity = self._get_similarity(
                    raw_out, out_sim
                )  # shape: parallel_eq_n,b,*,dim1,dim3
        ...
        if topk == 1:
            ...
            tmp_B_zero_point = torch.gather(
                B_zero_point_candidates, dim=0, index=best_index
            )
            self.B_quantizer.scale.data.copy_(
                tmp_B_scale.view(self.B_quantizer.scale.shape)
            )
            self.B_quantizer.zero_point.copy_(
                tmp_B_zero_point.view(self.B_quantizer.zero_point.shape)
            )
        return best_index
    ...
    def calculate_percentile_candidates(self, x, l=0.9, r=1.0):
        ...
        while tensor_too_large:
            try:
                uppers_candidates = torch.quantile(x_, pct.to(x_.device), dim=-1).mean(
                    dim=-1, keepdim=False
                )  # shape: 2,(heads or 1)
                lowers_candidates = torch.quantile(
                    x_, (1 - pct).to(x_.device), dim=-1
                ).mean(
                    dim=-1, keepdim=False
                )  # shape: 2,(heads or 1)
                ...
                mini_batch_size *= 2
                x_ = (
                    x_.view(x_.shape[0], mini_batch_size, -1)
                    if self.head_channel_wise
                    else x_.view(1, mini_batch_size, -1)
                )
        ...
        splits = torch.linspace(0, 1, steps=num_scale).cuda()[
            :, None, None, None, None
        ] * (delta_max - delta_min)
        scale_candidates = (delta_min + splits).repeat(num_zp, 1, 1, 1, 1) / (
            2 * self.B_quantizer.n_levels - 1
        )
        ...
        zero_point_candidates = zp_candidates.repeat_interleave(num_scale)[
            :, None, None, None, None
        ]
        zero_point_candidates = zero_point_candidates.repeat(
    ...
    def _fpcs(self, x, fpcs_width=16, steps=6, search_strategy=None):
        ...
        while remain_steps > 0:
            ...
            ).reshape(-1, *scale_candidates.shape[1:])
            zero_point_candidates = topk_zp_candidates.repeat_interleave(
                fpcs_new_cnt, dim=0
            )
            topk_index = search_strategy(
                self,
            ...
            if remain_steps > 1:
                topk_scale_candidates = torch.gather(
                    scale_candidates, dim=0, index=topk_index
                )
                topk_zp_candidates = torch.gather(
                    zero_point_candidates, dim=0, index=topk_index
                )
    ...
    def hyperparameter_searching(self):
        ...
        A_scale_candidates, A_zero_point_candidates = (
            self.calculate_percentile_candidates(self.raw_input[0].cuda())
        )
        B_scale_candidates, B_zero_point_candidates = (
            self.calculate_percentile_candidates(self.raw_input[1].cuda())
        )
        ...
        for e in range(self.search_round):
            if self.fpcs:
                self._fpcs(
                    self.raw_input[0].cuda(),
                    steps=self.steps,
                    search_strategy=AsymmetricallyBatchingQuantMatMul._search_best_A_scale,
                )
                self._fpcs(
                    self.raw_input[1].cuda(),
                    steps=self.steps,
                    search_strategy=AsymmetricallyBatchingQuantMatMul._search_best_B_scale,
                )
            else:
...
class PostSoftmaxAsymmetricallyBatchingQuantMatMul(AsymmetricallyBatchingQuantMatMul):
    ...
    def __init__(
        ...
        if quantizer == "log2":
            self.A_quantizer = Log2Quantizer(
                n_bits=A_bit, symmetric=False, channel_wise=False
            )
        elif quantizer == "logsqrt2":
            self.A_quantizer = LogSqrt2Quantizer(
                n_bits=A_bit, symmetric=False, channel_wise=False
            )
        elif quantizer == "adalog":
            self.A_quantizer = AdaLogQuantizer(
                n_bits=A_bit, symmetric=False, channel_wise=False
            )
            self.table = torch.tensor(
                [2 ** (-j / self.A_quantizer.r) for j in range(120)]
            )
    ...
    def _search_best_A_log_base(self, q_candidates=None, topk=1):
        if q_candidates is None:
            q_candidates = (
                torch.tensor([i for i in range(10, 11 + self.eq_n)])
                .cuda()
                .view(-1, 1, 1, 1, 1)
            )
        batch_similarities = []  # similarities, need to concatenate and calculate sum
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                A_sim = A.squeeze(0)
                A_sim_quant = torch.round(
                    -A_sim.log2() * self.A_quantizer.r / cur_q
                )  # shape: parallel_eq_n, b, heads, *
                mask = A_sim_quant >= 2 * self.A_quantizer.n_levels
                A_sim_quant = A_sim_quant.clamp_(0, 2 * self.A_quantizer.n_levels - 1)
                col_index = (
                    torch.remainder(A_sim_quant * cur_q, self.A_quantizer.r)
                    .round_()
                    .long()
                )
                A_sim = (
                    2 ** (-1 * torch.floor(A_sim_quant * cur_q / self.A_quantizer.r))
                ) * self.table.to(A.device)[col_index]
                A_sim[mask] = 0
                out_sim = A_sim @ B_sim  # shape: parallel_eq_n,b,*,dim1,dim3
                similarity = self._get_similarity(
                    raw_out, out_sim
